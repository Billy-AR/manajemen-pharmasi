...
export default function LaporanPage() {
  // --- STATE ---
  const [activeTab, setActiveTab] = useState<"sales" | "lowStock" | "expiring">(
    "sales"
  );
  ...
  const handleExportPDF = () => {
    ...
    if (activeTab === "sales") {
      ...
      const tableBody = lowStock.map((item) => [
        item.name,
        item.stock.toString(),
      ...
      const tableBody = expiringSoon.map((item) => [
        item.name,
        item.stock.toString(),
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Pusat wawasan performa bisnis dan kesehatan inventaris.
          </p>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`rounded-md px-4 py-1.5 text-sm font-medium transition-all ${
                activeTab === tab.id
                  ? "bg-white text-emerald-900 shadow ring-1 ring-black/5"
            ...
            <div className="grid gap-4 lg:grid-cols-4">
              ...
              <div className="rounded-xl border border-emerald-100 bg-white p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-medium uppercase text-emerald-600/70">
                      Total Pendapatan
                    </p>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={loading}
                      className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-800 disabled:opacity-50 shadow-md"
                    >
                      {loading ? "..." : "Filter"}
                    </button>
                    <button
                    ...
                    {loading ? (
                      ...
                      sales.map((sale) => (
                        <tr key={sale.id} className="hover:bg-slate-50/50">
                          ...
                          <td className="whitespace-nowrap px-6 py-4 text-right font-mono font-medium text-emerald-900">
                            {formatRupiah(sale.total)}
                          </td>
                        </tr>
                      ))
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700 disabled:opacity-50 shadow-sm"
              >
                <Icons.Download /> Export
              </button>
            </div>
            ...
            <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <select
                  ...
                  className="rounded-lg border border-slate-300 bg-slate-50 py-1.5 pl-3 pr-8 text-sm focus:border-rose-500 focus:ring-rose-500"
                >
                ...
                <button
                  onClick={loadExpiringSoonReport}
                  className="rounded-lg bg-white border border-slate-300 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50"
                >
                  Terapkan
                </button>
              </div>
                  ...
                  {expiringSoon.length === 0 ? (
                    <tr>
                      <td
                        ...
                        className="py-12 text-center text-slate-500"
                      >
                        Tidak ada item yang mendekati kadaluarsa dalam{" "}
                        {daysAhead} hari.
                      </td>
                    </tr>
                  ) : (
                    expiringSoon.map((item) => (
                      <tr key={item.id} className="hover:bg-rose-50/10">
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className="inline-flex items-center rounded-full bg-rose-100 px-2.5 py-0.5 text-xs font-bold text-rose-800 border border-rose-200">
                            URGENT
                          </span>
                        </td>
                      </tr>
